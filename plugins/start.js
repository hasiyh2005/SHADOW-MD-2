    ...
    latestNews.forEach(async (newsItem) => {
        ...
        if (!lastNewsTitles[groupId].includes(newsItem.title)) {
            await conn.sendMessage(groupId, { 
                text: `🗞️ *${newsItem.title}*\n${newsItem.content}\n${newsItem.date}\n> > ᴘᴏᴡᴇʀᴅ ʙʏ ꜱʜᴀᴅᴏᴡ ᴍᴅ ɴᴇᴡꜱ.ʟᴋ➤` 
            });
        ...
        if (isGroup) {
            ...
            if (isAdmin || isBotOwner) {
                if (!activeGroups[from]) {
                    ...
                    await conn.sendMessage(from, { text: "🗞️ 24/7 SHADOW - MD News Activated Successful.." });
                    ...
                    await conn.sendMessage(from, { text: "🗞️ 24/7 SHADOW - MD News Already Activated.." });
                }
            } else {
                await conn.sendMessage(from, { text: "❌ ඔබ shadow md bot owner කෙනෙකුවත් grup ඇඩ්මින් වරයෙකුවත් නොවේ !!" });
            }
        } else {
        ...
        if (isGroup) {
            ...
            if (isAdmin || isBotOwner) {
                if (activeGroups[from]) {
                    ...
                    await conn.sendMessage(from, { text: "❌ 24/7 SHADOW -MD News Deactivated NOW.." });
                    ...
                    await conn.sendMessage(from, { text: "❌ 24/7 SHADOW - MD News is not active in this group.." });
                }
            } else {
                await conn.sendMessage(from, { text: "❌ ඔබ shadow md bot owner කෙනෙකුවත් grup ඇඩ්මින් වරයෙකුවත් නොවේ !!" });
            }
